...
class ParameterSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ($dataset as $row) {
            $batchCode = 'BATCH-' . str_pad($row[0], 3, '0', STR_PAD_LEFT);
            $actualDays = $row[7];

            $batch = Batch::where('batch_id', $batchCode)->first();
            ...
            for ($logIndex = 0; $logIndex < $totalLogs; $logIndex++) {
                ...
                Parameter::create([
                    'batch_id' => $batch->id,
                    'ambient_temp' => $this->pickExact($ambientSeries, $logIndex),
                    'humidity' => $this->pickExact($humiditySeries, $logIndex),
                    'soil_temp' => $this->pickExact($soilTempSeries, $logIndex),
